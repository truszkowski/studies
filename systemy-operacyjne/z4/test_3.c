...
int main() {
	...
	for (test = 0; test < TESTS; ++test) {
		...
		fprintf(stdout, "=> TEST %d => limit(read): %dkB, limit(write): %dkB, data: %dB, avg: %fs, dev: %fs, opt: %f, len: %dparts\n", 
			test, limitsr[test], limitsw[test], sizes[test], info.avg, info.dev, 
			( limitsr[test] == 0 && limitsw[test] == 0 ? 0.0E0 :
				1.0E0/(
					( limitsr[test] ? 1.0E0/((double)limitsr[test]) : 0.0E0 )
					+
					( limitsw[test] ? 1.0E0/((double)limitsw[test]) : 0.0E0 ) 
				)
			), 
			info.len);
		fflush(stdout);
